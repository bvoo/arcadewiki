...
export function addToComparison(company: string, controller: string): boolean {
  ...
  if (comparison.some((c) => c.company === company && c.controller === controller)) {
    return false;
  }
...
export function removeFromComparison(company: string, controller: string): void {
  const comparison = getComparison();
  const filtered = comparison.filter((c) => !(c.company === company && c.controller === controller));
...
export function isInComparison(company: string, controller: string): boolean {
  const comparison = getComparison();
  return comparison.some((c) => c.company === company && c.controller === controller);
}
