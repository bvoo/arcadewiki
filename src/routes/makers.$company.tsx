...
export const Route = createFileRoute('/makers/$company')({
  component: MakerPage,
  head: ({ params }) => {
    const controllers = getAllControllerDocs().filter((d) => d.meta.company === params.company);
...
function MakerPage() {
  ...
  if (controllers.length === 0) {
    return (
      <div className="min-h-screen bg-background text-foreground">
        <SiteHeader />
        <div className="p-6">
          <h1 className="mb-4 font-bold text-xl">Maker Not Found</h1>
          <p className="mb-6 text-muted-foreground">No controllers found for "{company}".</p>
  ...
  const sortedControllers = [...controllers].sort((a, b) => b.meta.releaseYear - a.meta.releaseYear);

  const currentlySoldCount = controllers.filter((c) => c.meta.currentlySold).length;
  const avgPrice =
    controllers.filter((c) => c.meta.priceUSD).reduce((sum, c) => sum + (c.meta.priceUSD || 0), 0) /
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto max-w-7xl p-6">
        <div className="mb-8">
          <h1 className="mb-2 font-bold text-3xl">{makerName}</h1>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {sortedControllers.map((doc) => (
            <ControllerCard key={doc.slug} slug={doc.slug} {...doc.meta} />
          ))}
        </div>
      </div>
    </div>
